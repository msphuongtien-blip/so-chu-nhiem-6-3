...
function setRole(r){role=r;$('teacherTab').classList.toggle('active',r==='teacher');$('studentTab').classList.toggle('active',r==='student')}
...
async function login(){const msg=$('loginMsg');msg.textContent='';const {data,error}=await sb.auth.signInWithPassword({email:$('email').value.trim(),password:$('password').value});if(error){msg.textContent=error.message;return}await startSession(data.user)}
sb.auth.getSession().then(async ({data})=>{if(data.session)await startSession(data.session.user)});
function setupUI(){$('teacherNav').classList.toggle('hidden',role!=='teacher');$('studentNav').classList.toggle('hidden',role!=='student');const d=currentProfile.full_name||currentUser.email;$('who').innerHTML='<div class="avatar">'+esc(d.slice(0,1).toUpperCase())+'</div><div><b>'+esc(d)+'</b><div class="mini">'+(role==='teacher'?'GVCN':'Học sinh')+'</div></div>';$('sideWho').textContent=d;$('sideRole').textContent=role==='teacher'?'Giáo viên chủ nhiệm':'Học sinh';showPage(role==='teacher'?'dashboard':'sHome')}
...
function openStudentForm(s) {
    const edit=!!s;openModal(edit?'Chỉnh sửa học sinh':'Thêm học sinh','<div class="grid two"><div class="field"><label>Họ tên</label><input id="sfName" value="'+esc(s?.full_name||'')+'"></div><div class="field"><label>Mã HS</label><input id="sfCode" value="'+esc(s?.student_code||'')+'"></div></div><div class="grid two"><div class="field"><label>Email</label><input id="sfEmail" type="email" value="'+esc(s?.email||'')+'"></div><div class="field"><label>Mật khẩu ban đầu</label><input id="sfPass" type="password" placeholder="Chỉ khi tạo mới"></div></div><div class="grid three"><div class="field"><label>Ngày sinh</label><input id="sfBirth" type="date" value="'+esc(s?.birth_date||'')+'"></div><div class="field"><label>Giới tính</label><select id="sfGender"><option '+(s?.gender==='Nam'?'selected':'')+'>Nam</option><option '+(s?.gender==='Nữ'?'selected':'')+'>Nữ</option><option '+(s?.gender==='Khác'?'selected':'')+'>Khác</option></select></div><div class="field"><label>Tổ</label><input id="sfTeam" type="number" min="1" max="4" value="'+(s?.team||'')+'"></div></div><div class="field"><label>SĐT phụ huynh</label><input id="sfPhone" value="'+esc(s?.parent_phone||'')+'"></div><div class="field"><label>Mức hỗ trợ</label><select id="sfSupport"><option '+(!s?.support_level?'selected':'')+'>Không</option><option '+(s?.support_level==='Theo dõi'?'selected':'')+'>Theo dõi</option><option '+(s?.support_level==='Cần hỗ trợ'?'selected':'')+'>Cần hỗ trợ</option><option '+(s?.support_level==='Cần can thiệp'?'selected':'')+'>Cần can thiệp</option></select></div><div class="field"><label>Ghi chú đặc biệt</label><textarea id="sfNote" rows="3">'+esc(s?.special_note||'')+'</textarea></div><div class="grid two"><div class="field"><label>Điểm mạnh hiển thị cho học sinh</label><textarea id="sfStrength" rows="3">'+esc(s?.strength_note||'')+'</textarea></div><div class="field"><label>Nội dung cần cải thiện</label><textarea id="sfImprove" rows="3">'+esc(s?.improve_note||'')+'</textarea></div></div><div class="field"><label>Ghi chú tiến bộ</label><textarea id="sfProgress" rows="3">'+esc(s?.progress_note||'')+'</textarea></div><button class="btn primary" onclick="saveStudent('+(edit?"'"+s.id+"'":'null')+')">Lưu học sinh</button>')
}
async function saveStudent(id) {
    const p= {
        full_name:$('sfName').value.trim(),student_code:$('sfCode').value.trim()||null,email:$('sfEmail').value.trim(),birth_date:$('sfBirth').value||null,gender:$('sfGender').value,team:Number($('sfTeam').value)||null,parent_phone:$('sfPhone').value.trim()||null,support_level:$('sfSupport').value==='Không'?'':$('sfSupport').value,special_note:$('sfNote').value.trim(),strength_note:$('sfStrength').value.trim(),improve_note:$('sfImprove').value.trim(),progress_note:$('sfProgress').value.trim()
    };if(!p.full_name||!p.email) {
        alert('Cần họ tên và email.');return
    }
    if(id) {
        const {
            error
        }
        =await sb.from('students').update(p).eq('id',id);if(error) {
            alert(error.message);return
        }
    } else {
        const pass=$('sfPass').value;if(pass.length<6) {
            alert('Mật khẩu ban đầu tối thiểu 6 ký tự.');return
        }
        const {
            data,error
        }
        =await sb.functions.invoke('admin-create-student', {
            body: {
                ...p,password:pass
            }
        }
        );if(error||data?.error) {
            alert(error?.message||data.error);return
        }
    }
...
function exportCSV() {
    const csv='STT,Họ tên,Mã HS,Ngày sinh,Giới tính,Tổ,SĐT PHHS,Thi đua,Mức hỗ trợ,Ghi chú\n'+students.map((s,i)=>[i+1,s.full_name,s.student_code,s.birth_date,s.gender,s.team,s.parent_phone,s.competition_score,s.support_level,s.special_note].map(x=>'"'+String(x??'').replaceAll('"','""')+'"').join(',')).join('\n');const a=document.createElement('a');a.href=URL.createObjectURL(new Blob(['\ufeff'+csv], {
        type:'text/csv'
    }
    ));a.download='so-chu-nhiem-6-3.csv';a.click()
}
function getRandomPool(scope) {
    if(!scope||scope==='all')return students.slice();if(String(scope).startsWith('team')) {
        const team=Number(String(scope).slice(-1));return students.filter(s=>Number(s.team)===team)
    }
    return students.slice()
}
function chooseRandomCandidate(scope) {
    const pool=getRandomPool(scope);if(!pool.length)return null;const recent=new Set(randomHistory.slice(-Math.min(8,pool.length)).map(String));const fresh=pool.filter(s=>!recent.has(String(s.id)));const candidates=fresh.length?fresh:pool;return candidates[Math.floor(Math.random()*candidates.length)]
}
let randomRunning=false;let randomAudioContext=null;function soundEnabled() {
    return !$('randomSound')||$('randomSound').checked
}
function getAudioContext() {
    if(!randomAudioContext) {
        const C=window.AudioContext||window.webkitAudioContext;if(C)randomAudioContext=new C()
    }
    if(randomAudioContext?.state==='suspended')randomAudioContext.resume();return randomAudioContext
}
function playTone(freq,duration,type='sine',volume=.035,delay=0) {
    if(!soundEnabled())return;const ctx=getAudioContext();if(!ctx)return;const now=ctx.currentTime+delay;const osc=ctx.createOscillator();const gain=ctx.createGain();osc.type=type;osc.frequency.setValueAtTime(freq,now);gain.gain.setValueAtTime(.0001,now);gain.gain.exponentialRampToValueAtTime(volume,now+.015);gain.gain.exponentialRampToValueAtTime(.0001,now+duration);osc.connect(gain).connect(ctx.destination);osc.start(now);osc.stop(now+duration+.03)
}
function playSpinTick() {
    playTone(540,.055,'square',.018)
}
function playCountdown(n) {
    playTone(n===1?720:620,.11,'sine',.028);if(n===1)playTone(980,.14,'sine',.022,.08)
}
function playWinnerSound() {
    if(!soundEnabled())return;[523.25,659.25,783.99,1046.5].forEach((f,i)=>playTone(f,.22,'sine',.045,i*.09))
}
function speakStudent(name) {
    if(!soundEnabled()||!('speechSynthesis' in window))return;window.speechSynthesis.cancel();const u=new SpeechSynthesisUtterance('Mời em '+name);u.lang='vi-VN';u.rate=.9;u.pitch=1.03;u.volume=1;const voices=window.speechSynthesis.getVoices();const vi=voices.find(v=>/^vi(-|_)/i.test(v.lang));if(vi)u.voice=vi;window.speechSynthesis.speak(u)
}
function sleep(ms) {
