...
function compWeekInput(){
  /*
   * Public synchronous week resolver used by Competition renderers and forms.
   * Callers must receive the YYYY-MM-DD value, not a Promise.
   */
  const el=$('compWeekFilter');
  if(el&&!el.value) el.value=getCurrentWeekStart();
  const start=compWeekStart(el?.value);
  if(el) el.value=start;
  if($('compWeekRange')) $('compWeekRange').textContent=compWeekRange(start);
  return start;
}
