...
function exportCSV() {
    const csv='STT,Họ tên,Mã HS,Giới tính,Tổ,Thi đua,Mức hỗ trợ,Ghi chú\n'+students.map((s,i)=>[i+1,s.full_name,s.student_code,s.gender,s.team,s.competition_score,s.support_level,s.special_note].map(x=>'"'+String(x??'').replaceAll('"','""')+'"').join(',')).join('\n');const a=document.createElement('a');a.href=URL.createObjectURL(new Blob(['\ufeff'+csv], {
        type:'text/csv'
    }
