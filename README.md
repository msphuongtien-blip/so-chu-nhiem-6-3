...
Giai đoạn 2 đã được triển khai trên database hiện tại. Bảng `students` không còn `birth_date`, `parent_phone` và `email`; mã học sinh được chuẩn hóa từ `HS001 → 6301` đến `HS044 → 6344` và `student_id` không thay đổi. Danh sách học sinh vẫn giữ nguyên 44 bản ghi và `student_id` không thay đổi.
...
6. Supabase → `students`: xác nhận `birth_date`, `parent_phone`, `email` không còn; mã HS từ 6301 đến 6344; đủ 44 học sinh; `student_id` không đổi.
...
## Phase 2 — Xóa email học sinh

### Quy tắc
- `students.email` đã được loại bỏ khỏi database.
- Email học sinh không còn xuất hiện trong hồ sơ, form, CSV hoặc logic frontend.
- `student_code` được chuẩn hóa thành số thuần từ `6301` đến `6344`.
- Mapping giữ nguyên theo mã cũ: `HS001 → 6301` ... `HS044 → 6344`.
- `students.id` và các foreign key nghiệp vụ không thay đổi.

### Authentication
- Giai đoạn 2 chưa triển khai đăng nhập học sinh bằng Mã HS.
- Giao diện GVCN vẫn dùng email của tài khoản giáo viên trong Supabase Auth.
- Luồng học sinh `Mã HS + mật khẩu` và reset password bởi GVCN/Admin sẽ được triển khai ở Giai đoạn 3.

### Test Phase 2
1. Học sinh & hồ sơ: không còn trường Email, Ngày sinh hoặc PHHS.
2. Kiểm tra mã HS từ `6301` đến `6344`.
3. Xác nhận đủ 44 học sinh.
4. Xác nhận `student_id` không thay đổi.
5. Export CSV: không có Email, Ngày sinh, PHHS.
6. Supabase `students`: không còn các column `email`, `birth_date`, `parent_phone`.
7. Thêm/sửa hồ sơ: dùng Mã HS, không yêu cầu email.
8. Không kiểm thử login học sinh bằng Mã HS ở Phase 2; dành cho Phase 3.


## Phase 2 — Email và mã học sinh

- Email đã được loại bỏ khỏi `public.students` và không còn xuất hiện trong form hồ sơ hoặc CSV.
- Email của tài khoản GVCN trong Supabase Auth vẫn được giữ ở Phase 2 vì đây là cơ chế đăng nhập của giáo viên.
